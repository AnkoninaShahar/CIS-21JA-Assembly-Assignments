...
; DATE        : 1/16/2026
; DESCRIPTION : Filling out a prewritten assembly file with missing spaces.
...
main proc
   ...
   mov  edx, OFFSET prompt_2nd_int ; edited line [Prompts the 2nd integer using the edx register for I/O]
   call WriteString

   ; set input text color to yellow
   mov eax, yellow ; edited line [Moves 'yellow' to the eax register for storing calculations]
   call SetTextColor
   ...
   mov  edx, OFFSET prompt_text_color ; edited line [Prompts the text color using the edx register for I/O]
   call WriteString
   ...
   call Crlf
   ; reset text color to white
   mov eax, white ; edited line [Moves 'white' to the eax register for storing values]
   call SetTextColor

   ; add the inputs, then save it to sum
   mov   eax, operand_1
   add   eax, operand_2 ; edited line [Adds the eax register with operand_2 and stores it back in eax]

   ; save sum
   mov  sum, eax ; edited line [Moves the value in the eax register to the sum variable]
   ...
   ; display sum in HEX
   mov  edx, OFFSET hex_sum_str ; edited line [Outputs the value in hex_sum_str using edx for I/O]
   call WriteString
   ...
   ; set input text color
   mov eax, yellow ; edited line [Moves 'yellow' to the eax register for storing values]
   call SetTextColor ; edited line [Calls SetTextColor which sets the color of the text to the value in the eax register]
   ...
   call WriteString ; edited line [Calls WriteString which displays the value stored in edx?]
